...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem Vindo a família*${mdata.subject}*\nTAG OBRIGATÓRIA!
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5538988313755@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Este são os números bloqueados :\n'
				...
				case 'gtts':
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior parte do texto é nada')
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* PAULO.H\n*WPP:* wa.me/+5538988313755\n*INSTA:* paulo_h_stk\n\n\n*Digite *.off* '})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
